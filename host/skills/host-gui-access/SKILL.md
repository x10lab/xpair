...
- Don't click approval/permission dialogs yourself; use the `approve` skill (`xpair approve --for "..."`).

## Is the host app alive / granted? (avoiding misdiagnosis)
- The menu bar app (XpairHost) **runs even without permissions** — permissions only gate computer-use *actions*.
  If it doesn't appear in the menu bar immediately after `open` from the CLI, that doesn't mean it's "dead".
- Don't guess at liveness/permissions; check with one line:  `xpair status`
  (App liveness = per launchctl; AX/SR/FDA grants = per the `~/.xpair/host/logs/status.json` the app writes every second.)
