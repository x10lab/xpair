...
신규 `install/glue/bin/remote-pair-launch` 가 레퍼런스 `~/.claude/bin/claude-iterm-launch` 의 모든 동작을 재현하는지(1:1 연결 모델) 항목별로 검증한 결과. 테스트 근거: `tests/t_*.sh` (총 116 assertion, fail 0).
