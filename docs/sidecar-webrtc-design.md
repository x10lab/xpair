...
> 라이선스: AGPL-3.0-or-later (AGPL 혼입 금지)

---

## (a) 프로토콜 설계 참고 (요약)

외부 코드는 참조하지 않은 자체(클린룸) 설계다.
...
사이드카(screen): AGPL-3.0-or-later (Cargo.toml에 명시)
```

이 사이드카는 순수 자체 구현이다 — 외부 코드 복사 없이 공개 프로토콜 문서만 참고한 클린룸 설계.
...
본 설계는 공개 표준 문서·일반 WebRTC/NAT-traversal 지식만 참고한 **자체(클린룸) 구현**이다.
외부 카피레프트(GPL/AGPL) 코드는 일절 참조하지 않는다.
