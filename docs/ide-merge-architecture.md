...
| 화면 | `native/remote-pair-screen` — v1a(WS+JPEG) 완료, v1b(WebRTC) TODO | `remotepair-ext/media/remote-desktop.js` — 사이드카 WS 소비 웹뷰 |
...
| `remote-pair/native/remote-pair-screen/` | `native/remote-pair-screen/` | 프로토콜 상수/포맷을 `shared/screen-protocol/`로 추출 |
