...
| 화면 | `rs/screen` — v1a(WS+JPEG) 완료, v1b(WebRTC) TODO | `remotepair-ext/media/remote-desktop.js` — 사이드카 WS 소비 웹뷰 |
| 온보딩 | `client/cli/web` 마법사(role-aware 8/6 스텝) | walkthroughs 4종(+ patch로 VSCode 기본 온보딩 OFF) |
...
4. **런치 글루** — `client/cli/remote-pair-editor`·`remote-pair-desktop`.
...
├─ native/
│   └─ screen/         Rust 사이드카 (v1a WS+JPEG · v1b WebRTC)
...
| `remote-pair/rs/screen/` | `rs/screen/` | 프로토콜 상수/포맷을 `shared/screen-protocol/`로 추출 |
...
```
        ...
        ▼         ▼                            ▼                       ▼
   native/      client/cli/web/                host/                 ide/remotepair-ext/
