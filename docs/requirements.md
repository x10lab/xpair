...
> **Client onboarding = single app, single main process (decided 2026-06-16).** The client is **one app = one main process** (the RemotePair IDE). It must NOT be a separate onboarding Electron app that spawns the IDE (the standalone `client/onboarding` 2nd-process wrapper is removed).
> - On first run, the IDE's **main process opens the onboarding as its own separate BrowserWindow** (a pre-workbench window) — **not** a workbench editor tab.
> - **The workbench window is NOT shown during onboarding** (suppressed). On completion the onboarding window closes and the workbench opens → boots into Remote Desktop.
> - **Gating:** not onboarded → onboarding window first; already onboarded → straight to the workbench/RD (no onboarding window).
> - **vendor/vscodium stays pristine** — no direct/continuous edits to the subtree. The main-process change lives only as a **patch** (applied at build, trap-cleaned) so `git subtree pull` stays conflict-free; keep the vendor-side hook **surgical** (a minimal call hook; logic in a `remotepair/`-owned module) so an upstream VSCodium update carries the change forward via patch re-capture rather than a vendor fork.
