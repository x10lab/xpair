...
- **사이드카**(napi 애드온 아님): 신규 Rust 워크스페이스 `screen`(별도 디렉터리, out-of-band). 저결합 + Electron Node ABI 핀 회피 + 크래시 격리 + AGPL 프로세스-경계 방화벽.
...
- **Phase 5 ✅ (와이어프레임 레이아웃)** 사용자 와이어프레임에 맞춰 셸 레이아웃 재설계 — settings + 익스텐션으로 달성(소스패치 없이):
  - **Remote Desktop = 우측 에디터 영역의 핀 탭("RD")**. 좌측 액티비티바 WebviewView → `createWebviewPanel`(viewType `remotepair.remoteDesktop`) + `pinEditor`. 일반 파일 탭과 동급으로 열리고 normal 파일도 함께. 입력/스트리밍(v0·v1)은 동일.
  - **좌측 = 터미널 중심 minimal**: 1회성 `setupLayout`(globalState v2 가드)로 panel을 왼쪽(`positionPanelLeft`), 기본 사이드바·보조바 닫기, 터미널 오픈 → "좌 터미널(탭 다중) / 우 에디터(RD 핀 + 파일)".
  - `configurationDefaults`: 네이티브 타이틀바, 상태바 표시, `startupEditor=none`, panel 기본 left, 터미널 탭 좌측.
  - 검증: `.app` 재실행 screencapture — RD 핀 에디터 탭(호스트 화면 렌더)·좌측 zsh 터미널 탭·사이드바 닫힘·우측 보조바 없음·네이티브 상하 바 확인.
  - **남은 소스패치(미착수, last-resort)**: 좌측 rail을 기본 액티비티바 아이콘 → **"Terminal / Browser / Settings" 텍스트 전용**으로. 워크벤치 소스 수정 필요(리베이스 비용↑)이라 미룸. "Browser"(Simple Browser/claude-in-chrome) 항목도 동반 설계.
- **Phase 7 (스캐폴드)** Rust 사이드카 `rs/screen` (구 `native/screen` — 모노레포 통합으로 rs/로 이동) (scap/screencapturekit 캡쳐 + cargo-deny AGPL 방화벽). v1 webrtc 트랜스포트는 multi-week 남음. **v1a WS 서버 완료**(tungstenite, JPEG 프레임 ~10fps, 로컬 검증 483KB 프레임). **익스텐션 v1 WS 클라이언트 완료**(ssh -L 터널 → webview WebSocket → canvas, v0 스크린샷 폴링 폴백, auto 모드 4s 워치독). `client/cli/remote-pair-screen-deploy`로 호스트 배포(+ 호스트 Screen Recording grant는 수동 1회).
