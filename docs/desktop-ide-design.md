...
- **Phase 7 (스캐폴드)** Rust 사이드카 `native/remote-pair-screen` (scap/screencapturekit 캡쳐 + cargo-deny AGPL 방화벽). v1 webrtc 트랜스포트는 multi-week 남음. **v1a WS 서버 완료**(tungstenite, JPEG 프레임 ~10fps, 로컬 검증 483KB 프레임). **익스텐션 v1 WS 클라이언트 완료**(ssh -L 터널 → webview WebSocket → canvas, v0 스크린샷 폴링 폴백, auto 모드 4s 워치독). `client/remote-pair-screen-deploy`로 호스트 배포(+ 호스트 Screen Recording grant는 수동 1회).
