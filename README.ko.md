...
- **호스트 — Homebrew 설치(옵션 A):** Homebrew만 있으면 됩니다. tmux-aqua가 앱에 이미 임베드돼 있어 Xcode는 필요 없습니다.
- **호스트 — 소스 빌드(옵션 B)만:** Xcode Command Line Tools(또는 전체 Xcode) — tmux 정적 빌드용

---

## 설치

### 호스트 — 항상 켜져 있는 Mac (하나 선택)

**옵션 A — Homebrew (권장, 빌드 없음).** 앱은 self-signed이고 공증되지 않았기 때문에, 그냥 다운로드하면 첫 실행에서 Gatekeeper가 막습니다. Homebrew가 quarantine 플래그를 떼어주므로 앱이 정상 실행되고, 손쉬운 사용·화면 기록 권한도 안정적인 서명 정체성에 묶여 유지됩니다(TCC는 공증이 필요 없습니다 — quarantine만 없고 서명이 안정적이면 됩니다).

```bash
brew tap ghyeongl/remote-pair https://github.com/ghyeongl/remote-pair
brew install --cask remote-pair-host
open ~/Applications/RemotePairHost.app   # 첫 실행; 데몬을 자기설치
```

첫 실행 시 앱이 **데몬**(LaunchAgent, `~/.remote-pair`, tmux-aqua 링크, watchdog)을 스스로 설치하고 돌기 시작합니다. 여기까지는 데몬뿐 — approve 규칙·스킬과 `remote-pair` CLI까지 받으려면 호스트 설정(옵션 B)을 실행하세요. 앱은 순수 권한 데몬으로 두고, 스킬·규칙·CLI는 설정 쪽이 맡습니다(결합도를 낮추기 위함).

**옵션 B — CLI bootstrap (소스 빌드).** 바이너리를 믿기보다 직접 컴파일하려는 경우. Xcode CLT + Homebrew 필요.
```bash
curl -fsSL https://raw.githubusercontent.com/ghyeongl/remote-pair/main/shared/bootstrap.sh | ROLE=host bash
```

어느 쪽이든, 아래 [일회성 권한 부여](#일회성-권한-부여-호스트--물리적-화면-또는-vnc-필요)로 마무리하세요.

### SSH 접근 — 클라이언트에서 호스트로 키 기반 로그인

클라이언트는 호스트에 SSH로 붙으므로, 비밀번호 없는 키 인증을 한 번 설정해 둡니다(클라이언트 `onboard`와 `remote-pair doctor` 모두 이게 된다고 가정합니다).

**호스트에서** — 원격 로그인 켜기:

```bash
sudo systemsetup -setremotelogin on   # 또는: 시스템 설정 → 일반 → 공유 → 원격 로그인
```

**클라이언트에서** — 키가 없으면 만들고, 호스트로 복사한 뒤, 짧은 별칭을 등록:

```bash
[ -f ~/.ssh/id_ed25519 ] || ssh-keygen -t ed25519        # 이미 키가 있으면 생략
ssh-copy-id ghyeong@192.168.1.42                          # 호스트 계정명 @ 호스트 주소(LAN IP, 호스트명, Tailscale 이름)
```

매번 전체 주소를 치지 않도록 `~/.ssh/config`에 별칭을 추가하면 `ssh gh-mac-m1`로 붙을 수 있습니다:

```ssh-config
Host gh-mac-m1
    HostName 192.168.1.42      # LAN IP, mDNS 이름(mac-m1.local), 또는 Tailscale 이름
    User ghyeong
```
...
이 `gh-mac-m1` 별칭이 나중에 `remote-pair config set host gh-mac-m1`에 그대로 넣는 값입니다.

> LAN 밖에서 호스트에 붙어야 하나요? **[Tailscale](https://tailscale.com)** 같은 메시 VPN이 어디서나 통하는 안정적인 이름을 호스트에 주니 `HostName`을 그쪽으로 가리키세요. `mosh`와 함께 쓰면 네트워크가 끊겨도 attach가 살아남습니다.

### 클라이언트 — 직접 쓰는 노트북 (빌드 없음, Xcode 없음)

```bash
curl -fsSL https://raw.githubusercontent.com/ghyeongl/remote-pair/main/shared/bootstrap.sh | ROLE=client bash
```

Finder 빠른 동작 + `remote-pair` CLI를 설치하고, 이어서 `remote-pair onboard`(호스트 주소, 터미널 앱, 폴더 매핑)를 자동 실행합니다. 권한도 빌드도 없습니다.

### 일회성 권한 부여 (호스트) — 물리적 화면 또는 VNC 필요
