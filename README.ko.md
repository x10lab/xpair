...
설치는 **클라이언트 IDE**에서 시작합니다: 깔고, 실행하면, 첫 실행 온보딩이 나머지를 다 합니다 — CLI 설치, SSH 배선, 호스트에 엔진 설치·인증, 서명된 호스트 앱을 호스트로 푸시까지. 각 단계는 막다른 골목 대신 **스스로 고치는 hard-gate**입니다. 자동화할 수 없는 단 하나는 호스트 물리 화면에서의 권한 부여입니다.
...
Xpair를 엽니다. 첫 실행 시 온보딩(별도 창이 아닌 IDE 내장)이 열리고 다음을 진행합니다:
...
CLI를 선호하거나, 호스트를 그 자체 화면에서 셋업하고 싶다면? bootstrap 스크립트와 `xpair install-host`가 같은 일을 합니다:

```bash
# 클라이언트: CLI + Finder 빠른 동작만 (`xpair onboard` 자동 실행):
curl -fsSL https://raw.githubusercontent.com/x10lab/xpair/main/shared/bootstrap.sh | ROLE=client bash

# 호스트: 그 자체 화면에서 셋업 (CLI + approve glue + brew cask로 XpairHost.app):
curl -fsSL https://raw.githubusercontent.com/x10lab/xpair/main/shared/bootstrap.sh | ROLE=host bash

# 또는 이미 설정된 클라이언트에서 호스트 앱을 푸시:
xpair install-host --host <user@host>
```

앱만, CLI 없이: `brew install --cask xpair-host` (위 tap 후).
