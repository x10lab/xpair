#!/usr/bin/env bash
# uninstall-host.sh — remove XpairHost from a target Mac over SSH (the reverse of
# `xpair install-host`), for repeatable install/uninstall testing of the onboarding flow.
#
...
#
# Removes (user-level, no sudo): the LaunchAgents + plists, ~/Applications/XpairHost.app, the
# host config (~/.xpair/host/host.env) and the staged build cache. A /Applications/*.app copy needs
# admin and is reported (not force-removed) so this stays sudo-free and safe.
#
# Usage: uninstall-host.sh --host <ssh-target> [-y] [--force]
set -euo pipefail

HOST=""; YES=0; FORCE=0; APP_NAME="XpairHost"
...
ssh_t 'bash -s' <<'REMOTE'
  set -u
  U="$(id -u)"
  for L in com.x10lab.xpair-host com.x10lab.xpair-host-watchdog \
           com.ghyeong.xpair-host com.ghyeong.xpair-host-watchdog \
           com.x10lab.xpair com.x10lab.xpair-watchdog \
  ...
  pkill -f "XpairHost" 2>/dev/null || true
  pkill -f "tmux-aqua"      2>/dev/null || true
  rm -rf "$HOME/Applications/XpairHost.app" 2>/dev/null || true
  rm -f  "$HOME/.xpair/host/host.env" 2>/dev/null || true
  rm -rf "$HOME/.cache/xpair" 2>/dev/null || true
  if [ -d /Applications/XpairHost.app ]; then
    echo "⚠︎ /Applications/XpairHost.app needs admin to remove — left in place (run 'sudo rm -rf' on that Mac if intended)."
  fi
  echo "removed user-level XpairHost (app/agents/config)"
REMOTE

echo "✅ uninstalled $APP_NAME from $HOST. Re-run 'xpair install-host --host $HOST' to reinstall."
