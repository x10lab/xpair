#!/usr/bin/env bash
# Xpair IDE — upstream sync (Vendor separation / Option C).
#
# vendor/vscodium/ is a git subtree tracking PRISTINE VSCodium (github.com/VSCodium/vscodium,
# remote 'vscodium'). Xpair-owned files live in remotepair/ and NEVER enter the tracked
...
cat <<'EOF'
Xpair IDE upstream sync (Option C)
=======================================
...
To pull a newer VSCodium recipe into the vendor subtree:
  ...
  • Xpair surface (remotepair/) is untouched by the pull.
  • After pulling, rebuild (./build.sh) and re-run shared/check-ide-selfcontained.sh.
  • If the new recipe drops/renames something the Xpair frontend patch relies on,
