# Xpair surface manifest — `client/ide/` (VSCodium fork, Vendor separation / Option C)

> **What this is:** `client/ide/` is a VSCodium-based IDE split into two halves:
> - `remotepair/` — **everything Xpair owns** (this directory). The only files we author/maintain.
> - `vendor/vscodium/` — **pristine VSCodium** build recipe, tracked as a `git subtree` from
>   github.com/VSCodium/vscodium (remote `vscodium`). **Inviolable** — never edit; any Xpair
>   change to a stock file must be expressed here in `remotepair/` (as a patch/overlay/orchestrator).
>
> **Why (Option C):** vendor tracks *pristine* VSCodium directly, so Xpair files never enter
> the tracked subtree → `git subtree pull` stays conflict-free **by construction**. (The earlier
> standalone `remotepair-ide` fork — which carried Xpair files inside the recipe — is retired.)

---

## 1. Xpair-owned surface (everything under `remotepair/`)
...
The **single** Xpair-authored patch (the entire G001–G009 workbench frontend). At build time
...
Xpair files and modifies 18 stock workbench files (all marked `// Xpair:`):
...
Xpair identity (nameShort=Xpair, applicationName=xpair, darwinBundleIdentifier
`com.x10lab.xpair`, urlProtocol=xpair, …). At build time `build.sh` places it at
...
= pristine VSCodium `dev/build.sh` with the Xpair identity env (APP_NAME, ASSETS_REPOSITORY,
...
**VSCodium's own** de-branding/onboarding patches, NOT Xpair's), `icons/`, `stores/`, `upstream/`,
...
> Attribution correction (vs the pre-restructure manifest): the ONLY Xpair deviations from
> pristine VSCodium are the four §1 items (ext, zz patch, product overlay, dev-build.sh). `00-brand`,
> `80`, `81` were previously mislabeled as Xpair — they are stock and live in `vendor/`.

## 2.5 Licensing
- **`remotepair/` (Xpair-owned) = AGPL-3.0-or-later** — matches the monorepo root `LICENSE`.
...
- **`vendor/vscodium/` = MIT** (VSCodium/VS Code upstream) — we cannot relicense it; it stays as
  shipped. The distributed Xpair.app combines the two: AGPL governs the whole derivative work,
...
- **Upstream pull (Option C):** `git subtree pull --prefix=client/ide/vendor/vscodium vscodium <tag> --squash`.
  Current anchor: VSCodium `1.121.03429` (VS Code 1.121.0, MS commit `987c9597…`). Xpair files are
...
5. **vendor/vscodium/ is pristine** — verify with `git diff vscodium/<tag> -- client/ide/vendor/vscodium`
   (expect no Xpair content). Any needed stock change becomes a `remotepair/patches/*` patch.
