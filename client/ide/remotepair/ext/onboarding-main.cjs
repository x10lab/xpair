...
function openOnboardingWindow({ electron, onComplete }) {
  ...
  // Closing the onboarding window without completing must not open the workbench. The client is only
  // handed off after the renderer sends onboarding:complete from the gated Done step.
  _win.on('closed', () => {
    _win = null
  })
  return _win
