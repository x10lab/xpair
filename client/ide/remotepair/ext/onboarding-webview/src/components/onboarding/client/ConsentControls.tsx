...
// Shared first-run consent UI — two INDEPENDENT opt-in checkboxes (product analytics + crash
// reports), BOTH default OFF. Rendered on the FIRST onboarding screen (StepWelcome) so consent is
// decided BEFORE the activation funnel fires, and again on StepDone as a re-toggle summary. With
// both OFF, no telemetry or crash report ever leaves the machine. Persists to client.env via the
// bridge (TELEMETRY_CONSENT / CRASH_REPORT_CONSENT); a single source of truth for both surfaces.
...
export function ConsentControls({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = await window.remotepair.tGetConsent();
        if (active && c) {
          setTelemetryOn(!!c.telemetry);
          setCrashOn(!!c.crashReport);
        }
      } catch {
  ...
  // Persist on every toggle (both flags written together; bridge upserts client.env).
  const persist = (t: boolean, c: boolean) => {
    setTelemetryOn(t);
    setCrashOn(c);
    try {
      void window.remotepair.tSetConsent(t, c);
    } catch {
