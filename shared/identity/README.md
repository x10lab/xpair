...
RemotePair 모노레포(`remote-pair` 코어 · `ide/` VSCodium 포크 · `rs/` 네이티브 엔진)의
...
| 소비자 | 검증 항목 |
|--------|-----------|
| `ide/product.json` | nameShort/Long·applicationName·dataFolderName·darwinBundleIdentifier·urlProtocol·server*·win32* |
...
## 버전 정책
컴포넌트는 성숙도가 달라 독립적으로 버전이 오른다(host 0.4.x = 성숙, ide/rs 0.1.0 = 초기).
...
소비자 값을 **생성/주입**하도록 확장 가능(특히 `ide/` self-containment 시 — 별도 스토리).
